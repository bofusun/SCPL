...
def make_env(
	...
	assert mode in {'train', 'color_easy', 'color_hard', 'video_easy', 'video_hard', 'video_black','distracting_cs'}, \
	...
	if not is_distracting_cs and mode!="video_black":
		env = VideoWrapper(env, mode, index, seed)
	elif not is_distracting_cs and mode=="video_black":
		env = BlackVideoWrapper(env, mode, index, seed)
	
	env = FrameStack(env, frame_stack)
	if not is_distracting_cs:
		env = ColorWrapper(env, mode, index, seed)

	return env

class BlackWrapper(gym.Wrapper):
	"""Wrapper for the color experiments"""
	def __init__(self, env, mode, index, seed=None):
	...
	def get_random_color(self):
		...
		return {'grid_rgb1': np.array([0., 0., 0.]), 'self_rgb': np.array([1., 1., 1.]), 'skybox_rgb': np.array([0. , 0. , 0.]), 'grid_rgb2': np.array([0. , 0., 0.])}
...
class ColorWrapper(gym.Wrapper):
	...
	def reset(self):
		...
		if 'color' in self._mode:
			...
			if self._mode == 'video_hard' or self._mode == 'video_black':
				setting_kwargs['grid_rgb1'] = [.2, .8, .2]
				setting_kwargs['grid_rgb2'] = [.2, .8, .2]
				setting_kwargs['grid_markrgb'] = [.2, .8, .2]
			self.reload_physics(setting_kwargs)
...
def do_green_screen_black(x, bg):
	"""Removes green background from observation and replaces with bg; not optimized for speed"""
	assert isinstance(x, np.ndarray) and isinstance(bg, np.ndarray), 'inputs must be numpy arrays'
	assert x.dtype == np.uint8 and bg.dtype == np.uint8, 'inputs must be uint8 arrays'
...
class VideoWrapper(gym.Wrapper):
	...
	def __init__(self, env, mode, index, seed):
		gym.Wrapper.__init__(self, env)
		self._mode = mode
		self._seed = seed
		self._random_state = np.random.RandomState(seed)
	...
	def _get_video_paths(self):
		video_dir = os.path.join('/root/deeplearningnew/sun/rl_generalization/dmcontrol-generalization-benchmark-main/src/env/data', self._mode)
...
class BlackVideoWrapper(gym.Wrapper):
	"""Green screen for video experiments"""
	def __init__(self, env, mode, index, seed):
		gym.Wrapper.__init__(self, env)
		self._mode = mode
	...
	def _get_video_paths(self):
		video_dir = os.path.join('/root/deeplearningnew/sun/rl_generalization/dmcontrol-generalization-benchmark-main/src/env/data', self._mode)
		if 'video_easy' or 'video_black' in self._mode:
			video_dir = os.path.join('/root/deeplearningnew/sun/rl_generalization/dmcontrol-generalization-benchmark-main/src/env/data', 'video_easy')
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(10)]
		elif 'video_hard' in self._mode:
			self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(100)]
		else:
			raise ValueError(f'received unknown mode "{self._mode}"')
		print("self._video_paths",self._video_paths)
	...
	def _greenscreen(self, obs):
		"""Applies greenscreen if video is selected, otherwise does nothing"""
		if 'video' in self._mode:
			bg = self._data[0] # select frame
			bg = np.zeros(bg.shape)
			print("bg.shape",bg.shape)
			bg = self._interpolate_bg(bg, obs.shape[1:]) # scale bg to observation size
			return do_green_screen_black(obs, bg) # apply greenscreen
		return obs
