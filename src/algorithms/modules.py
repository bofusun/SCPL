...
# 转移模型
class My_TransitionModel(nn.Module):
    def __init__(self, encoder_feature_dim, action_shape, layer_width):
        ...
        self.fc_reward = nn.Linear(layer_width, 1)
    ...
    def forward(self, x):
        ...
        sigma = None
        return mu, sigma, reward

    def sample_prediction(self, x):
        mu, sigma, reward = self(x)
        return mu, reward
...
class Combined_Transition_Model(nn.Module):
    def __init__(self, model1, model2):
        super(Combined_Transition_Model, self).__init__()
        self.model1 = model1
        self.model2 = model2

    def forward(self, x, y):
        x = self.model1(x)
        x = self.model2(torch.cat([x, y], dim=1))
        return x