...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, batch_size, device, prefill=True, prioritized=False):
        ...
        if prioritized:
            ...
            self.priority = torch.zeros(capacity, device=device)
            self.max_priority = 1
        else:
    ...
    def _get_idxs(self, n=None):
        if n is None:
            n = self.batch_size
        if self.prioritized:
            csum = torch.cumsum(self.priority[:self.capacity if self.full else self.idx], 0)
            val = torch.rand(size=(n,), device=self.device)*csum[-1]
            self.ind = torch.searchsorted(csum, val).cpu().data.numpy()
        else:
            self.ind = np.random.randint(0, self.capacity if self.full else self.idx, size=n)
        return self.ind
    ...
    def reset_max_priority(self):
        self.max_priority = float(self.priority[:self.capacity if self.full else self.idx].max())
    ...
    def sample_soda(self, n=None):
        ...
        return torch.as_tensor(obs).to(self.device).float() #.cuda().float()

    def __sample__(self, n=None):
        idxs = self._get_idxs(n)

        obs, next_obs = self._encode_obses(idxs)
        obs = torch.as_tensor(obs).to(self.device).float() #.cuda().float()
        next_obs = torch.as_tensor(next_obs).to(self.device).float() #.cuda().float()
        actions = torch.as_tensor(self.actions[idxs]).to(self.device) #.cuda()
        rewards = torch.as_tensor(self.rewards[idxs]).to(self.device) #.cuda()
        not_dones = torch.as_tensor(self.not_dones[idxs]).to(self.device) #.cuda()
